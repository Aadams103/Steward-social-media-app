...
// ==========================================
// STEWARD PERSONA CONFIGURATION
// ==========================================

export type StewardVoiceIdentity = 'neutral' | 'feminine' | 'masculine';

export type StewardPersonalityStyle = 
  | 'calm_formal'      // Default: Calm & Formal
  | 'warm_supportive'  // Warm & Supportive
  | 'direct_executive' // Direct & Executive
  | 'analytical_precise'; // Analytical & Precise

export interface StewardPersona {
  id: string;
  name: string; // Default: "Steward", fully user-defined
  voiceIdentity: StewardVoiceIdentity;
  personalityStyle: StewardPersonalityStyle;
  updatedAt: Date;
}

export const STEWARD_PERSONALITY_STYLES: { 
  id: StewardPersonalityStyle; 
  label: string; 
  description: string 
}[] = [
  { 
    id: 'calm_formal', 
    label: 'Calm & Formal', 
    description: 'Professional, restrained, clear communication with measured risk signaling' 
  },
  { 
    id: 'warm_supportive', 
    label: 'Warm & Supportive', 
    description: 'Friendly, encouraging, approachable while maintaining professionalism' 
  },
  { 
    id: 'direct_executive', 
    label: 'Direct & Executive', 
    description: 'Concise, decisive, action-oriented communication' 
  },
  { 
    id: 'analytical_precise', 
    label: 'Analytical & Precise', 
    description: 'Data-driven, detailed, methodical explanations' 
  },
];

// ==========================================
// BRAND SYSTEM (My Brand Outputs)
// ==========================================
...
export interface BrandStrategy {
  ...
  coreValues: string[];   // Fundamental principles
  brandArchetype: string; // Brand personality archetype
}
...
export interface MarketPositioning {
  ...
  competitiveLandscape: string; // Analysis of competitors
  uniqueValueProposition: string; // What makes this brand unique
...
export interface BrandIdentity {
  visualDirection: {
    logoUsage?: string;      // Guidelines for logo usage
    colorIntent: string[];   // Color palette with intent/meaning
    typographyIntent: string; // Typography direction (not raw fonts)
  };
  verbalIdentity: {
    tone: string;            // Overall tone
    vocabulary: string[];    // Preferred words/phrases
    messagingRules: string[]; // Rules for messaging
  };
...
export interface BrandExperience {
  ...
  touchpoints: string[];     // All brand touchpoints
  consistencyRules: string[]; // Rules for maintaining consistency
}

// Brand/Business Profile - Complete brand system generated by Steward
export interface BrandProfile {
  ...
  // The four canonical brand artifacts (generated by Steward)
  brandStrategy: BrandStrategy;
  marketPositioning: MarketPositioning;
  brandIdentity: BrandIdentity;
  brandExperience: BrandExperience;

  // Legacy fields (kept for backward compatibility, may be migrated)
  audience?: string; // Deprecated: use marketPositioning.targetPersonas
  brandVoice?: string; // Deprecated: use brandIdentity.verbalIdentity.tone
  writingDos?: string[]; // Deprecated: use brandIdentity.verbalIdentity.messagingRules
  writingDonts?: string[]; // Deprecated: use brandIdentity.verbalIdentity.messagingRules
  offers?: string[]; // May be derived from brandStrategy
  services?: string[]; // May be derived from brandStrategy
  prices?: string;
  ctaLinks?: { label: string; url: string }[];
  primaryColors?: string[]; // Deprecated: use brandIdentity.visualDirection.colorIntent
  secondaryColors?: string[]; // Deprecated: use brandIdentity.visualDirection.colorIntent
  visualVibe?: string; // Deprecated: use brandIdentity.visualDirection
  doNotUseImagery?: string[]; // May be part of brandIdentity.visualDirection
  postingGoals?: PostingGoal[]; // May be derived from brandStrategy
  contentPillars?: string[]; // May be derived from brandStrategy
  bannedWords?: string[]; // May be part of brandIdentity.verbalIdentity.messagingRules
  bannedClaims?: string[]; // May be part of brandIdentity.verbalIdentity.messagingRules
  complianceNotes?: string; // May be part of brandExperience.consistencyRules
...
// ==========================================
// USER PROFILE TYPES (The Human Identity)
// ==========================================

// User Profile - The individual human operator
...
// User Preferences (Human, not Brand)
...
export interface DeviceHistory {
  ...
  browser?: string;
  lastSeenAt: Date;
  action: 'login' | 'logout' | 'password_change' | '2fa_enabled' | '2fa_disabled';
}
